...
if($this_id){
	...
	$sql = "select A.mb_no,A.mb_id,A.mb_recommend, B.mb_no as recommend_no, depth+1 as mb_depth from (SELECT mb_no,mb_id, mb_recommend FROM g5_member ) A, g5_member B where A.mb_recommend = B.mb_id ORDER By mb_no ASC";
	...
	for ($i=0; $row=sql_fetch_array($result); $i++) {
			...
			$sql_update = "update g5_member set depth = '".$row['mb_depth']."' where mb_id ='".$row['mb_id']."'<br>";
