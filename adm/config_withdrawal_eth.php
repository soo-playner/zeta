...
$sql = " select count(*) as cnt from pinna_btc_trans A inner join g5_member M on A.mb_id = M.mb_id WHERE 1=1 and type = 4 ";
$sql .= $sql_condition;
...
$sql = "select * from pinna_btc_trans A inner join g5_member M on A.mb_id = M.mb_id WHERE 1=1 and type = 4";
$sql .= $sql_condition;
...
<script>
	$(function(){
		$('.regTb [name=status]').on('change',function(e){
			$.post( "/adm/config_withdrawal_eth.u.php", {
				uid : $(this).attr('uid'),
				status : $(this).val()
			}, function(data) {
				alert('상태 변경 성공.')
...
<form name="fsearch" id="fsearch" class="local_sch01 local_sch" action="./config_withdrawal_eth.php" method="GET">
	상태 : 
	...
	<input type="text" name="mb_hp" placeholder="전화번호" class="frm_input" value="<?=$_GET[mb_hp]?>" />
	...
	<table cellspacing="0" cellpadding="0" border="0" class="regTb">
		...
		<?for ($i=0; $row=sql_fetch_array($list); $i++) {?>
			<tr>
				...
				<?if($row[it_pool5_profit]<0){?>
				<td style="color:red"><?=$row[it_pool5_profit]?></td>
				<?}else{?>{
				<td><?=$row[it_pool5_profit]?></td>
				<?}?>
				<td><?=$row[total_mining]?></td>
				<td><?=$row[total_wid]?></td>

				<td><?=$row[mb_hp]?></td>
				<td><a href="#" onclick="window.open('https://etherscan.io/address/<?=$row[addr]?>','width=800, height=500');"><?=$row[addr]?></a></td>
				<td><?=$row[amt]?></td>
				<td><?=$row[create_dt]?></td>
				<?if($row[status]!='N'){?>
				<td align='center'>
				...
				<?}?>
				<td align='center'><?=$row[update_dt]?></td>
			</tr>
		<?}?>
        </tbody>
    </table>
