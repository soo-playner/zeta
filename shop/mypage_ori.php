...
<!-- ##start## pkCalendar ##### -->
	...
	?>
		...
		<div class="day">
			...
			<?
			/*##  ################################################*/
			$qry = sql_query(" select * from g5_shop_cart where mb_id = '{$member['mb_id']}' and ct_time like '".$year."-".sprintf("%02d",$month)."-".sprintf("%02d",$day)."%' and ct_status != '쇼핑' and ct_status != '주문'   ");
			while ($res = sql_fetch_array($qry)) {
			?>
			<p><a href="/shop/orderinquiryview.php?od_id=<?=$res['od_id']?>"><?=$res['it_name']?>(<?=$res['ct_qty']?>)</a></p>
			<?
			}
			/*@@End.  #####*/
			?>
