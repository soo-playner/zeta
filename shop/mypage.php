...
.infoBx ul li span {display:inline-block;*display:inline;*zoom:1;color:#000;padding-left:20px;width:100px;background-color:rgba(39,48,62,0.05);margin-right:20px;}
	...
	<div class="infoBx">
		...
		<ul>
			...
			?>
			<li id='qr_code'><span>QR코드</span>
				<img id="qrImg" src="https://chart.googleapis.com/chart?chs=100x100&cht=qr&chl=100x100&chld=M|0&cht=qr&chl=otpauth://totp/Pinnacle (<?=$member['mb_id']?>)%3Fsecret%3D<?=$encoded?>" />
			</li>
			<li><span>OTP 인증</span>
				<input type="text" maxlength="6" style="width:100px;margin-right:10px;" placeholder='인증번호' id="otp" />
				<a href="javascript:setOtpKey();" id="otpKey"  class="btn"><?=$member['otp_flag'] != Y ? '적용' : '취소'?></a>
			</li>
		</ul>
	</div><!-- // infoBx -->
...
.calendar_year {position:relative;color:#333;font-size:30px;text-align:center;line-height:70px;}
#peak_caution {left:50%;margin-top:-70px;margin-left:130px;font-size:14px;line-height:70px;}
#peak_caution span {color:#000;}
#prev_month {position:absolute;padding:0;width:80px;line-height:70px;text-align:left;z-index:100;}
#next_month {position:absolute;right:0;padding:0;width:80px;line-height:70px;text-align:right;z-index:100;}
...
table.calendar tbody td {padding:10px;vertical-align:top;background-color:#F5F5F5;text-align:right;font-size:13px;height:60px;}
table.calendar tbody th {}
...
.items {line-height:18px;}
.items p {margin-bottom:1px;text-align:left;}
...
<!-- ##start## pkCalendar ##### -->
	...
	?>
		...
		<div class="day">
			...
			?>
			<p><a href="/shop/mybenefit.php?mb_id=<?=$member['mb_id']?>"><?=$res['dv_gubun']?>(<?=number_format($res['dv_money'])?>)</a></p>
			<?
			}
			/*@@End.  #####*/
			?>
...
?>
	...
	<div class="infoBx">
        ...
        $limit = " limit 0, 5 ";
        include G5_SHOP_PATH.'/benefit_list.php';
        ?>

			<div class="smb_my_more">
				<a href="./mybenefit.php" class="btn01">수당내역 더보기</a>
			</div>
		</div>
	</div><!-- // infoBx -->
...
// otp 발급용 비밀키 생성. 아이디로 생성함. 
function setOtpKey()
{
	$.post( "/otp/otp_update.php", {otp : $('#otp').val()}, function(data) {
		//alert(data);
		data = $.parseJSON(data);
		if(data.otp_flag == 'Y'){
			$('#otpKey').html('취소');
