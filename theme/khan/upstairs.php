...
<main>
	<div class="container upstairs">
					...
					<div class="r_card_wrap ">
						...
						if(count($row) == 0) {
							...
							array_push($data_arr, array(
								...
								"it_price"=>$row[$i-1]['it_point'],
								"it_supply_point"=>$row[$i-1]['it_supply_point']
							));

							if($row[$i-1]['it_id'] == '2021051031'){
						...
						?>
							<div class="<?=$row_col?> r_card_box">
								<div class="r_card r_card_<?=$i-1?>" data-row=<?=json_encode($data_arr,JSON_UNESCAPED_UNICODE)?>>
									<p class="title"><?=$row[$i-1]['it_name']?></p>
									<div class="b_blue_bottom"></div>
									<div class="text_wrap">
										<div class="it_price">$ <?=Number_format($row[$i-1]['it_point'])?></div>
										<div class='origin_price'>= ￦<?=Number_format($usd_price * $row[$i-1]['it_point'])?></div>
									</div>
								</div>
							</div>
						<?}
		...
		for($i = 0; $i < count($ordered_items); $i++){		
			$color_num = substr($ordered_items[$i]['it_name'],1,1); 
			if(count($ordered_items) > 3){$spread_average = 3;}else{$spread_average = 1;}
		?>		

			<div class="content-box3 product_buy_wrap pack_<?=$color_num-1?> col-11">
				<li class="row">
					<p class="title col-5"><?=strtoupper($ordered_items[$i]['it_name'])?></p>
					<!-- <p class='num col-7'>PV : <?=$ordered_items[$i]['pv']?> </p> -->
				</li>
				<li class="row">
					<p class="value col-8"><?=$ordered_items[$i]['od_time']?></p>
				</li>
			</div>
		...
		<div class="history_box content-box mt40">
			...
			<div class="hist_con">
				<div class="hist_con_row1">
					...
					<div class="row">
						<h2 class="pack_name pack_f_<?=substr($row['od_name'],1,1)?>"><?= strtoupper($row['od_name']) ?> </h2>
...
$(function(){
	...
	$('#go_wallet_btn').click(function(e){
		if(won_price > 0){
			go_to_url('mywallet'+'&sel_price='+won_price);
		}else{
			go_to_url('mywallet');
		}
	});
